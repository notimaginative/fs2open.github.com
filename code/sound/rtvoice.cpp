...
static unsigned int Rtv_callback_time;			// callback time in ms
...
static unsigned char	*Encode_buffer1 = nullptr;
...
static size_t Decode_buffer_size;

/////////////////////////////////////////////////////////////////////////////////////////////////
// RECORD/ENCODE
/////////////////////////////////////////////////////////////////////////////////////////////////

Uint32 TimeProc(Uint32 interval, void *param __UNUSED)
{
	if ( !Rtv_callback ) {
...
int rtvoice_init_recording(int  qos)
{
	...
	if ( Encode_buffer1 ) {
		...
		Encode_buffer1=nullptr;
	}

	Encode_buffer1 = reinterpret_cast<unsigned char*>(vm_malloc(Rtv_capture_raw_buffer_size));
	Assert(Encode_buffer1);

	if ( Encode_buffer2 ) {
		vm_free(Encode_buffer2);
		Encode_buffer2=nullptr;
	}

	Encode_buffer2 = reinterpret_cast<unsigned char*>(vm_malloc(Rtv_capture_raw_buffer_size));
	...
	if ( Rtv_capture_raw_buffer ) {
		...
		Rtv_capture_raw_buffer=nullptr;
	}

	Rtv_capture_raw_buffer = reinterpret_cast<unsigned char*>(vm_malloc(Rtv_capture_raw_buffer_size));
	...
	if ( Rtv_capture_compressed_buffer ) {
		...
		Rtv_capture_compressed_buffer=nullptr;
	}

	Rtv_capture_compressed_buffer_size=Rtv_capture_raw_buffer_size;	// be safe and allocate same as uncompressed
	Rtv_capture_compressed_buffer = reinterpret_cast<unsigned char*>(vm_malloc(Rtv_capture_compressed_buffer_size));
...
void rtvoice_stop_recording()
{
	...
	// clear any leftover capture samples
	int rc = 0;

	do {
		rc = dscap_get_raw_data(Rtv_capture_raw_buffer, Rtv_capture_raw_buffer_size);
	} while (rc > 0);
...
// exit:	0	=>	success
//			!0	=>	failure
int rtvoice_start_recording( void (*user_callback)(), unsigned int callback_time )
{
	if ( !dscap_supported() ) {
		return -1;
	}
	...
	if ( user_callback ) {
		Rtv_record_timer_id = SDL_AddTimer(callback_time, TimeProc, nullptr);
...
int rtvoice_16to8(unsigned char *data, int size)
{
	...
	for (i=0; i<size; i+=2) {
		...
		sample16 += 32768;
		sample8 = static_cast<unsigned char>(sample16>>8);
...
void rtvoice_get_data(unsigned char **outbuf, int *compressed_size, int *uncompressed_size, double *gain, unsigned char **outbuf_raw, int *outbuf_size_raw)
{
	...
	*uncompressed_size=raw_size;
	...
	if ( Rtv_do_compression ) {
		csize = rtvoice_compress(Rtv_capture_raw_buffer, raw_size, Rtv_capture_compressed_buffer, static_cast<int>(Rtv_capture_compressed_buffer_size));
	...
	if (outbuf_raw != nullptr) {
		*outbuf_raw = Rtv_capture_raw_buffer;
	}
	if (outbuf_size_raw != nullptr) {
...
size_t rtvoice_get_decode_buffer_size()
{
...
void rtvoice_close_playback()
{
	...
	if ( Rtv_playback_uncompressed_buffer ) {
		...
		Rtv_playback_uncompressed_buffer=nullptr;
	}
...
int rtvoice_init_playback()
{
	...
	Decode_buffer_size = static_cast<size_t>(rtvf->frequency * (RTV_BUFFER_TIME) * static_cast<int>(rtvf->bits_per_sample/8.0f));
	...
	Decode_buffer = reinterpret_cast<unsigned char*>(vm_malloc(Decode_buffer_size));
	...
	Rtv_playback_uncompressed_buffer = reinterpret_cast<unsigned char*>(vm_malloc(Rtv_playback_uncompressed_buffer_size));
...
sound_handle rtvoice_play_compressed(int index, unsigned char *data, int size, int uncompressed_size, double gain)
{
	int ds_handle;
	...
	Assert(static_cast<size_t>(uncompressed_size) <= Rtv_playback_uncompressed_buffer_size);
...
// exit:	>=0	=>	handle to playing sound
//			-1		=>	error, voice not played
sound_handle rtvoice_play_uncompressed(int index, unsigned char* data, int size)
{
	int ds_handle;
