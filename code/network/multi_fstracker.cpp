...
void multi_stats_fs_to_tracker(scoring_struct *fs, vmt_stats_struct *vmt, player *pl, int tracker_id)
{
	...
	for (int idx = 0; idx < MAX_SHIP_CLASSES; ++idx) {
		if (fs->kills[idx] > 0) {
			vmt->num_ships++;
		}
	}
	...
	const int count = std::min(static_cast<unsigned short>(vmt->num_medals + vmt->num_ships), MAX_FS2OPEN_COUNTS);
...
void multi_stats_tracker_to_fs(vmt_stats_struct *vmt,scoring_struct *fs)
{
	...
	fs->last_flown = (_fs_time_t)vmt->last_flown;
	if(fs->last_flown < 0){
		fs->last_flown = 0;
	}
...
void multi_fs_tracker_check_dup_callsign(net_player *player,int player_index)
{
	...
	for(idx=0;idx<MAX_PLAYERS;idx++){
		...
		if(MULTI_CONNECTED(Net_players[idx]) && !MULTI_STANDALONE(Net_players[idx]) && !MULTI_PERM_OBSERVER(Net_players[idx]) && (Net_players[idx].tracker_player_id != -1)){
			SDL_assert(strcmp(player->m_player->callsign,Net_players[idx].m_player->callsign));
		}
	}
}
...
void multi_fs_tracker_report_stats_results()
{
	...
	for(idx=0; idx<MAX_PLAYERS; idx++){		
		...
		if(MULTI_CONNECTED(Net_players[idx]) && !MULTI_STANDALONE(Net_players[idx])){
			...
			if(((Net_players[idx].tracker_player_id <= 0) || (Net_players[idx].flags & NETINFO_FLAG_MT_GET_FAILED) || (Net_players[idx].flags & NETINFO_FLAG_MT_SEND_FAILED) || !(Net_players[idx].flags & NETINFO_FLAG_MT_DONE)) && (Net_players[idx].m_player != NULL)){
